...
L'espérance de la différence statistique entre l'ensemble $J_{x_V,l}^{unif}$ choisi uniformément et l'ensemble $J_{x_V, l}^{\mathbf{H}_U}$ choisi parmi les bons ensembles pour $\mathbf{H}$ peut être majorée comme suit :
...
\noindent De plus, les ensembles $\J$ étant inclus dans un ensemble d'information, l'aléa interne de \verb|DecodeU| et de \verb|VarDecodeU| ne dépendent pas du choix de $J_{x_V,l}^{unif}$ et $J_{x_V, l}^{\mathbf{H}_U}$. 
